...
'''defines various output types from the bike model
'''

import csv, copy
from tables import openFile,Filters,Float32Atom
from bikeAssign.path_trace import path_trace
from bikeAssign.choice_set.path_size import path_size
from bikeAssign.misc import get_time_dependent_variable, get_orig_path

__author__ = "Lisa Zorn & Jeff Hood, San Francisco County Transportation Authority"
__license__= "GPL"
...
def create_csv_from_matrix(matrix,master_config,name=""):
	...
	matrix_file=open(master_config.output_config['matrix_data'],'w')
...
def create_h5_from_logsums(logsums,maxutil_components,master_config):
    """ 
    logsums in a matrix
    maxutil_components is a dictionary: component_name => [mat0, mat1, ...].
      Only outputs for mat0
    """

    outfile      = openFile(master_config.output_config['logsum_data'], mode="w")
    compfilt     = Filters(complevel=1,complib='zlib')
    table        = outfile.createCArray(where=outfile.root, name="BikeLogsums",
                                        atom=Float32Atom(),
                                        shape=logsums.shape,
                                        title="BikeLogsums",
                                        filters=compfilt)
    ...
    outfile      = openFile(master_config.output_config['maxutil_data'], mode="w")
    compfilt     = Filters(complevel=1,complib='zlib')
    for component_name in maxutil_components:
        table        = outfile.createCArray(where=outfile.root, 
                                            name=component_name,
                                            atom=Float32Atom(),
                                            shape=logsums.shape,
                                            title=component_name,
                                            filters=compfilt)
        table[:]    = maxutil_components[component_name][0]        
    outfile.close()
...
def create_csv_from_assignment(network,master_config):
	...
	writer=csv.writer(file) # default lineterminator='\r\n'
...
from bikeAssign.choice_set.link_elimination import link_elimination
